...
python scripts/run_sweep.py --write-manifest       # 691 conditions
...
`pytest -q` → **96 passed, 1 skipped**. The skip is `tests/test_loader.py`, which needs `mne`; with mne installed it runs, and its data-dependent cases skip until the EDFs are downloaded. Nothing in the experiment path (cache → runner → analysis) imports mne.
